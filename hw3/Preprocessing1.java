...
public class Preprocessing1 {
	...
	public static void brownProcessing(String input_file, String output_file, float percentage, 
		...
		for (int i = 0; i < num_genres; i++) {
			genreDir[i] = new File(input_file + "\\" + genreDirName[i]);
			System.out.println(genreDir[i].getName());
		}

		System.out.println("111");

		List<List<List<String>>> tree_bank_by_genre = new ArrayList<List<List<String>>>();
		for (int i = 0; i < num_genres; i++) {
			Concatenate(genreDir[i], tree_bank_by_genre.get(i));
		}
		...
		for (int i = 0; i < num_genres; i++) {
			int genre_size = tree_bank_by_genre.get(i).size();
			int train_size = Math.round(genre_size * percentage);
			int count = 0;
			for (List<String> tree : tree_bank_by_genre.get(i)) {
				count ++;
				if (count <= train_size)
					tree_bank_train.add(tree);
				else
