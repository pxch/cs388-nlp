...
public class TokenAccuracyEvaluator extends TransducerEvaluator
{
	...
	private static HashSet<String> lexicon = new HashSet<String>();
	...
	public void evaluateInstanceList (TransducerTrainer trainer, InstanceList instances, String description) 
  {
		...
		for (int i = 0; i < instances.size(); i++) {
			...
			if (description.equals("Training")) {
				for (int j = 0; j < input.size(); j++) {
					lexicon.add(input.get(j).toString());
				}
			}
			...
			if (description.equals("Training")) {
				...
				for (int j = 0; j < trueOutput.size(); j++) {
					boolean OOV = !lexicon.contains(input.get(j).toString());
					totalTokens++;
					if (OOV)
						totalOOVTokens++;
					if (trueOutput.get(j).equals(predOutput.get(j))) {
		...
		if (description.equals("Testing")) {
			double oov_perc = ((double)totalOOVTokens)/totalTokens;
			double oov_acc = ((double)numCorrectOOVTokens)/totalOOVTokens;
			accuracy.put("Testing OOV Percentage", oov_perc);
			accuracy.put("Testing OOV Accuracy", oov_acc);
			logger.info ("Testing OOV percentage="+oov_perc);
			logger.info ("Testing OOV accuracy="+oov_acc);
		}
	}
