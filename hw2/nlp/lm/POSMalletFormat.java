...
public class POSMalletFormat {
    ...
    public POSMalletFormat(File file) {
	this.file = file;
	try {
	...
	for (int i = 0; i < files.length; i++) {
		...
		if (!file.getName().contains("CHANGES.LOG"))
		    sentences.addAll(new POSMalletFormat(file).tokenLists());
	    }
	    else 
	    {
	...
	public static void writeToMalletFormat(File[] files, File output_file) {
	...
	File output_file = new File(args[args.length - 1]);
//	List<List<String>> sentences = 	convertToTokenLists(files);	
	writeToMalletFormat(files, output_file);	
